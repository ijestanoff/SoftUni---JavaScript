...
<section id="editPage">
    <form @submit=${createSubmitHandler(onEdit)} data-id=${item._id} class="theater-form">
        <h1>Edit Theater</h1>
        <div>
...
async function onEdit({title, date, author, imageUrl, description}, event) {
    if (!title || !imageUrl || !description || !date || !author ) {
        return alert('All fields are required!');
    }
    const id = event.dataset.id;
    await editItem(id, title, date, author, imageUrl, description);
